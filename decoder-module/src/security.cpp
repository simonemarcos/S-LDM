...
etsiDecoder::GNDataRequest_t
Security::createSecurePacket (etsiDecoder::GNDataRequest_t dataRequest, bool &isCertificate)
{
...
Security::extractSecurePacket (etsiDecoder::GNDataIndication_t &dataIndication, storedCertificate_t &certificateData) {
    ...
    if (present1 == Ieee1609Dot2Content_PR_signedData) {
        ...
        if (present3 == SignerIdentifier_PR_digest) {
            ...
            m_digest=asn1cpp::getField(signedDataDecoded->signer.choice.digest,std::string);
            m_digest.clear();
            for (int i = 0; i < signedDataDecoded->signer.choice.digest.size; i++)
            {
                std::stringstream ss;
                ss << std::hex << std::setw(2) << std::setfill('0') << (int)signedDataDecoded->signer.choice.digest.buf[i];
                //ss <<c_hash[i];
                m_digest += ss.str();
            }
